...
export const getVerdict = (r) => {
  ...
  if (r.httpCode === '429' || r.rateLimit?.wasRateLimited) return 'Overloaded'
...
function computeQoS(r) {
  ...
  // If explicitly rate-limited by proxy, penalize heavily (treat as down/overloaded)
  if (r.rateLimit?.wasRateLimited) return 0
...
export function parseArgs(argv) {
  ...
  const json = args.some(a => a.toLowerCase() === '--json')
