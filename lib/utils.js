...
export const getAvg = (r, windowMs = 10 * 60 * 1000) => {
  const now = Date.now()
...
export function findBestModel(results) {
  const eligible = results.filter(r => r.status !== 'banned' && r.status !== 'disabled')
...
export function parseArgs(argv) {
  ...
  if (command === 'autostart') {
    const positionalAction = args.find((a, idx) => idx > 0 && ['install', 'uninstall', 'status'].includes(a.toLowerCase()))
