...
function configureOpenClaw(port) {
  ...
  // Intelligent check: update if exists, otherwise create
  ...
  // Only set primary if not set, or if it was previously modelrelay/auto-fastest
  // This avoids overwriting user's explicit choice if they already have OpenClaw set up differently
  if (!config.agents.defaults.model.primary || config.agents.defaults.model.primary === 'modelrelay/auto-fastest') {
      config.agents.defaults.model.primary = 'modelrelay/auto-fastest'
  }

  if (!config.agents.defaults.models) config.agents.defaults.models = {}
  // Ensure the alias/config block exists
